...
public12()
{
  ...
  fd1 = open("/testsymlink4/r", O_CREATE | O_RDWR);
  if(fd1 < 0) fail("failed to open r");

  r = symlink("/testsymlink2", "/testsymlink3/q");
  if(r < 0)
    fail("symlink q -> p failed");

  r = symlink("/testsymlink4", "/testsymlink2/p");
  if(r < 0)
    fail("symlink p -> 3 failed");

  fd2 = open("/testsymlink3/q/p/r", O_RDWR);
  if(fd2<0) fail("Failed to open /testsymlink3/q/p/r\n");
