...
sys_open(void)
{
  ...
  if(omode & O_CREATE){
    ...
    if (ip->type == T_SYMLINK && !(omode & O_NOFOLLOW)){
      int threshold = 20, cnt = 0;
      while(cnt < threshold && ip -> type == T_SYMLINK){
        int l = 0;
        readi(ip, 0, (uint64)&l, 0, sizeof(int)); //read length
        if (l > MAXPATH){
          return -1;
        }
        readi(ip, 0, (uint64)path, sizeof(int), l + 1);
        iunlockput(ip);
        ip = namei(path);
        if (!ip){
          end_op();
...
sys_chdir(void)
{
  ...
  //printf("type:%d, symlink:%d\n",ip->type, T_SYMLINK);
  /*
  int len_path_new = strlen(path); 
  for (int i = 0; i < len_path_new - len_path; i++){
    path[i] = path[len_path + i];
  }
  path[len_path_new - len_path] = '\0';
  */
...
sys_symlink(void)
{
  ...
  begin_op();
  struct inode *i_node = create(path, T_SYMLINK, 0, 0);
  if (!i_node){
    end_op();
    return -1;
  }
  int l = strlen(target);
  writei(i_node, 0, (uint64)&l, 0, sizeof(int));
  writei(i_node, 0, (uint64)target, sizeof(int), l + 1);
  iupdate(i_node);
  iunlockput(i_node);
  end_op();
  return 0;
