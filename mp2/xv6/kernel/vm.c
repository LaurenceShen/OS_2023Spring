...
void vmprint(pagetable_t pagetable) {
	...
	for (int i = 0; i < 512; i ++){
		...
		if (!(*pte&PTE_V)){
			...
			if (*pte & PTE_S){
				printf("+-- %d: pte=%p va=%p blockno=%p V", i, pte, va, pa);
			}
			else{
				printf("+-- %d: pte=%p va=%p pa=%p V", i, pte, va, pa);
			}
			if (*pte & PTE_R){
				printf(" R");
			}
			...
			for (int j = 0; j < 512; j++){
				...
				if (!(*pte&PTE_V)){
					...
					if (*pte & PTE_S){
						...
						printf("+-- %d: pte=%p va=%p blockno=%p V", j, pte, va, pa);
					}
					else{
						if (i != last1) printf("|   ");
						else printf("    ");
						printf("+-- %d: pte=%p va=%p pa=%p V", j, pte, va, pa);
					}
					if (*pte & PTE_R){
						printf(" R");
					}
					...
					for (int k = 0; k < 512; k++){
						...
						if (*pte & PTE_V) {
							...
							if (*pte & PTE_S){
								...
								printf("+-- %d: pte=%p va=%p blockno=%p V", k, pte, va, pa);
							}
							else{
								if (i != last1) printf("|   ");
								else printf("    ");
								...
								printf("+-- %d: pte=%p va=%p pa=%p V", k, pte, va, pa);
							}
							if (*pte & PTE_R){
								printf(" R");
							}
							...
							printf("+-- %d: pte=%p va=%p blockno=%p", k, pte, va, pa);
...
int madvise(uint64 base, uint64 len, int advice) {
	...
	while(PGROUNDDOWN(base) + (uint64) k * PGSIZE < PGROUNDUP((uint64) base + len)){
		uint64 va = PGROUNDDOWN(base + (uint64) k*PGSIZE);
		...
		pte_t *pte = walk(myproc()->pagetable, va, 0);
			if (*pte & PTE_S) {
				...
				*/
				...
				// Create a PTE from a BLOCKNO and flag bits 
				mappages(myproc()->pagetable, va, PGSIZE,(uint64) pa, PTE_FLAGS(*pte));
		...
		pte_t *pte = walk(myproc()->pagetable, va, 0);
			if (*pte & PTE_V){
				...
				*pte &= ~PTE_V;
				*pte |= PTE_S;
				uvmunmap(myproc()->pagetable,va,PGSIZE,1);
				//kfree((char*)pa);
				flag = 1;
			}
			//else return -1;
